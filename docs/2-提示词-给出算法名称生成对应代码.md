...
Step 2) 在该目录下新建文件：MLDLAlgorithmCode/{folder_name}/PLAN.md
...
- 如果 MLDLAlgorithmCode/{folder_name}/ 已存在：
  ...
  - 仍然要确保 PLAN.md 存在：
    - 若 PLAN.md 不存在：新建
    - 若 PLAN.md 已存在：新建 PLAN_update_YYYY-MM-DD.md（或在原文件末尾追加一段 Update 也可以）
  - 在对话中说明你采取了哪种方式。
...
【必须写入文件】
- 在目录：MLDLAlgorithmCode/{folder_name}/ 下创建并写入：ALGO_SPEC.md
...
【必须写入文件】
- MLDLAlgorithmCode/{folder_name}/IMPLEMENTATION_DESIGN.md

【输入约束】
- 必须读取并引用 ALGO_SPEC.md（complexity_level/experiments/metrics/figures/data contract/DoD）
...
【输入约束】
- 必须遵循 ALGO_SPEC.md（experiments/metrics/figures/data contract/DoD/default_data_mode=synthetic）
- 必须遵循 IMPLEMENTATION_DESIGN.md（目录/文件职责/CLI/输出规范）
...
【本阶段目标】
- 对照 ALGO_SPEC.md 的 DoD 逐条验收当前实现；缺项必须修复，直到全部通过。
- 最终交付：运行命令 + 产物位置清单 + 简短结果解读。

【验收清单（必须逐条打勾汇报）】
1) 默认数据可跑通（synthetic）
2) 用户数据可跑通（CSV/Excel + target_col）
3) 至少 1 指标输出（固定格式与位置）
...
【最终交付（必须输出）】
A) 默认demo命令 + 用户CSV/Excel命令
B) 指标/图/模型等产物路径
C) 3-5句解读：展示了哪些特性点、对照说明什么价值/局限
```
